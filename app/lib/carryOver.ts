...
// ★ 문서 읽기는 전부 **readUserDoc**(재확인 포함) 이다.
//   이어붙이기의 모든 판단("옛 계정이 비었나?", "이 날짜 무드가 이미 있나?")은 읽기 결과에
//   달려 있는데, auth 가 막 바뀐 직후의 읽기는 멀쩡한 문서를 **"없음"으로 돌려준다**
//   (2026-07-29 실측, 경위는 lib/firestoreRead.ts 주석).
//   "비었다"고 잘못 읽으면 **묻지도 않고 덮어쓴다** — 이 파일이 절대 하지 않기로 한 바로 그 일이다.
//   읽기가 실패하면 throw 되고, 호출부(CarryOverDialog)가 잡아서 **아무것도 안 쓴다.**
//   그게 옳다: 모를 땐 건드리지 않는다.
import { collection, doc, getDocsFromServer, setDoc, writeBatch } from "firebase/firestore";
...
export async function applyCarryOver(snap: AnonSnapshot): Promise<CarryOutcome> {
  ...
  //
  // ★ 순서가 중요하다 — 병렬로 읽지 않는다.
  //   먼저 옛 계정 문서를 **재확인 읽기**로 가져와 토큰 교체 창을 벗어난 것을 확인한다.
  //   그 뒤에 moods 를 읽어야 믿을 수 있다. 병렬로 읽으면 moods 가 거짓 "비어 있음"으로 와서
  //   **기존 무드를 덮어쓴다** — 무드는 옛 것이 이겨야 하는데 정반대가 된다.
  // ★ consent 가 올 때까지 기다린 뒤에 판단한다.
  //   로그인 직후엔 linkedProvider 만 담긴 **부분 문서**가 온다(2026-09-05 진단).
  //   그걸 그대로 믿으면 사주가 있는 계정을 "빈 계정"으로 보고 익명 데이터를
  //   **덮어쓴다** — 무드는 옛 것이 이겨야 하는데 정반대가 된다.
  const userSnap = await readUserDoc(uid, undefined, "consent");

  // 기다렸는데도 문서에 **다른 필드는 있고 consent 만 없다** → 아직 안 자리 잡은 상태로 본다.
  //   (문서가 통째로 없는 건 진짜 첫 로그인이므로 그대로 진행한다 — 덮어쓸 것이 없다.)
  if (userSnap.exists() && (userSnap.data()?.consent === undefined)) {
    console.warn("[carryover] 옛 계정 문서가 아직 부분적임 — 이어붙이기 보류(덮어쓰기 방지)", uid);
    return { kind: "none" };
  }
...
export async function captureConsent(): Promise<Consent | null> {
  ...
  if (!u) return null;
  try {
    const snap = await readUserDoc(u.uid);
